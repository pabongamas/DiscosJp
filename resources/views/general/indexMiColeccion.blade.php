            ...
            <div class="col-12 col-lg-12 col-md-12" style="overflow: auto;padding-top: 15px;">
                <div class="row">
                    @if (count($albums) > 0)
                        @foreach ($albums as $album)
                            <div class="col-12 col-lg-4 col-md-4">
                                <div class="card"
                                    ...
                                    <img onclick="window.location='{{ route('albums.show', ['album' => $album->id_album, 'artista' => $album->id_artista]) }}'"
                                        class=" card-img-top cursorPointer" src="{{ $album->image }}">
                                    <div class="card-body">
                                        ...
                                        {{-- <a
                                            ...
                                            href="{{ route('albums.show', ['album' => $album->id_album, 'artista' => $album->id_artista]) }}">IR</a>
                                        --}}
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <span>No se encontraron Albumes registrados</span>
                    @endif
                </div>
            </div>
            @isset($pagination)
                        ...
                        <div class="col-12 col-lg-6 col-md-12">
                            <table class="tableAlbums">
                                <thead>
                                    <tr>
                                        <td>
                                            <label id="nombreAlbum"
                                                style="font-size: 25px;font-weight: bold;display:block;"></label>
                                            <label id="nombreArtista"
                                                style="font-size: 20px;font-weight: bold;display:block;"></label>
